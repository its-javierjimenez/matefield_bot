...
with open('d:/proyectos_dev/server_rcon_automation/apps/discord_bot/src/plugins/match.py', 'r', encoding='utf-8') as f:
    content = f.read()

new_class = '''class MatchPlayers:
...
# Regex to replace the entire MatchPlayers class
pattern = r'class MatchPlayers:\n(?:\s+.*?\n)+?(?=\s*@plugin\.include|\s*@crescent\.command|$)'
new_content = re.sub(pattern, new_class + '\\n\\n', content)

with open('d:/proyectos_dev/server_rcon_automation/apps/discord_bot/src/plugins/match.py', 'w', encoding='utf-8') as f:
    f.write(new_content)
