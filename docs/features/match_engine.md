...
Dado que los servidores de juego basados en ciertos motores no siempre tienen Webhooks o eventos Push nativos configurados para todas las acciones, el motor utiliza un modelo de **Polling**.
...
### 1. Inicio de Partida
El motor mantiene en memoria el ID de la partida actual (`current_match_id`). Si detecta que no hay partida actual, crea un nuevo registro en la tabla `Match` con el nombre del mapa actual y establece el `start_time`.

### 2. Actualización de Estadísticas (Stats)
Durante cada ciclo de polling (10 segundos):
- Se actualizan las estadísticas de los equipos (`MatchTeamStats`) calculando los puntajes basados en una métrica interna (ej: suma de métricas de los jugadores o tickets reportados por RCON).
...
2. **Rotación por Tiempo o Admin (Rotación)**:
   - El motor compara el índice de rotación actual reportado por RCON (`rotation_index`) con el índice guardado en memoria.
   - Si difieren, significa que el servidor cambió de mapa (ya sea porque se acabó el tiempo límite del juego o un Admin forzó el cambio).
   - **Victoria**: El motor lee las estadísticas de `MatchTeamStats`, selecciona al equipo con mayor puntaje, lo designa como ganador y establece el `end_time`. (Si la partida termina 0-0, se considera empate y el ganador queda en `NULL`).

3. **Cierre Forzado (Emergency Close)**:
   - En caso de reinicios de la API, partidas "zombies" que quedaron abiertas pueden ser cerradas por scripts de mantenimiento asegurando que el `end_time` no se superponga con la siguiente partida.
...
- Los Admins pueden iniciar un monitoreo sobre un jugador sospechoso (`/admin monitor`).
- El bot guarda las `kills` actuales del jugador y el `start_time`.
...
- Cuando el jugador abandona la partida, el monitoreo se cierra automáticamente.
